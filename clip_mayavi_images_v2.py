...
import glob
import os
path = "/home/manojpc/PointRCNN/imagelist/"
list_of_files = glob.glob(path + '*') # * means all if need specific format then *.csv
latest_file = max(list_of_files, key=os.path.getctime)
idx = int(latest_file[len(path)-len(latest_file):-6])


parser = argparse.ArgumentParser(description=" processing arguements ")
parser.add_argument("-s", "--startnum",  type=int, default=0, help=" The frame number to start processing from")
...
sorted_list = list(range(sorted_list[-1]))
if args.startnum == 0:
    numx = idx
else:
    numx = args.startnum
sorted_list = sorted_list[numx:]

time_result = list()

mlab.options.offscreen = True

print("starting from: {}".format(sorted_list[0]))
...
for each in tqdm(sorted_list):
    ...
    fig = mlab.figure(figure=None, bgcolor=(0,0,0), fgcolor=None, engine=None, size=(1920, 1080))
...
#     print(np.array(boxes_pred).shape)
    ...
    mlab.savefig('/home/manojpc/PointRCNN/imagelist/{}_0.png'.format(PC_ID), size=(1920, 1080), magnification = 3)
    
    scene.scene.x_plus_view()
    cam.zoom(2.8)
    # scene.scene.save('/home/manojpc/snapshotx.png',size=(1600,1400))
    mlab.savefig('/home/manojpc/PointRCNN/imagelist/{}_1.png'.format(PC_ID), size=(1920, 1080), magnification = 3)
    # mlab.savefig("myfigurex.jpg", size=(1600,1400),figure=fig)
    scene.scene.y_plus_view()
    cam.zoom(1.9)
    mlab.savefig('/home/manojpc/PointRCNN/imagelist/{}_2.png'.format(PC_ID), size=(1920, 1080), magnification = 3)
    # mlab.savefig("myfigurey.jpg", size=(1600,1400),figure=fig)
    scene.scene.z_plus_view()
    cam.zoom(2.2)
    mlab.savefig('/home/manojpc/PointRCNN/imagelist/{}_3.png'.format(PC_ID), size=(1920, 1080), magnification = 3)
